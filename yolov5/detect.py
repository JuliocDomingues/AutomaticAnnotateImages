...
path_annotations = str(FILE.parents[1]) + r'\\Annotations\\'
path_images = str(FILE.parents[1]) + r'\\Images\\'
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    dict = {"label": names[int(cls)], "points": [], "group_id": None, "shape_type": "rectangle", "flags": {}}
                    if save_txt:  # Write to file
                        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4))).view(-1).tolist()  # normalized xywh
                        line = (cls, *xywh, conf) if save_conf else (cls, *xywh)  # label format

                        dict["points"].append([int(xyxy[0]), int(xyxy[1])])
                        dict["points"].append([int(xyxy[2]), int(xyxy[3])])
